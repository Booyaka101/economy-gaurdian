      ...
      window.addEventListener('keydown', (e) => {
        try {
          ...
          if (isDbg) {
            ...
            try {
              svcShowToast && svcShowToast(`Debug ${next === '1' ? 'enabled' : 'disabled'}`);
            } catch {}
            e.preventDefault();
            return;
          }
          // Ctrl+Shift+R: toggle eg_debug_render
          const isRenderDbg =
            e.key && e.key.toLowerCase() === 'r' && e.ctrlKey === true && e.shiftKey === true;
          if (!typing && isRenderDbg) {
            let next = '1';
            try {
              const cur =
                typeof localStorage !== 'undefined' &&
                localStorage.getItem('eg_debug_render') === '1';
              next = cur ? '0' : '1';
              localStorage.setItem('eg_debug_render', next);
            } catch {}
            try {
              svcShowToast && svcShowToast(`Render debug ${next === '1' ? 'enabled' : 'disabled'}`);
            } catch {}
            e.preventDefault();
            return;
          }
          ...
          const isCopyTsm =
            e.key && e.key.toLowerCase() === 'g' && e.ctrlKey === true && e.shiftKey === true;
          if (!typing && isCopyTsm) {
            try {
