...
const ITEMS = arg('items', process.env.WATCH_ITEMS || '221754,152512,190394,190312')
  .split(',')
  .map((s) => Number(s.trim()))
  .filter(Boolean);
...
async function main() {
  ...
  for (let i = 1; i <= ITERS; i++) {
    ...
    if (!r) {
      log('[refresh-until-change] no response');
      continue;
    }
    log('[refresh-until-change]', {
      changed: r.changed,
      before: r.before?.lastFetched,
      after: r.after?.lastFetched,
    });

    // Quick fingerprint output for context
    const fp = await getJson(`${BASE}/debug/auctions/fingerprint`);
    if (fp) {
      log('[fingerprint]', fp.now || fp);
    }

    // Preview totals
    const prev = await getJson(`${BASE}/debug/auctions/delta/preview-raw`);
    if (prev) {
      log('[delta/preview-raw]', prev);
    }
    ...
    if (
      prev &&
      ((prev.itemTotalDrops || 0) > 0 ||
        (prev.priceBucketDrops || 0) > 0 ||
        (prev.endedCount || 0) > 0)
    ) {
      hit = true;
    }
    ...
    if (hit) {
      log('=== Delta detected. Stopping watcher ===');
      break;
    }
  }
