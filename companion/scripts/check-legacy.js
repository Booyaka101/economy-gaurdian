...
function main(){
  ...
  for (const f of files) {
    ...
    for (const rx of FORBIDDEN) {
      if (rx.test(txt)) {violations.push({ file: f, pattern: rx.toString() })}
    }
    ...
    if (hasDomBinding && !isController) {
      ...
      if (isServiceWorker) {
        ...
        const isOnlySwReg = /window\.addEventListener\(\s*['"]load['"]/.test(txt) && /navigator\.serviceWorker\.register\(/.test(txt)
        if (!isOnlySwReg) {
          violations.push({ file: f, pattern: 'DOM event binding outside controller (*.controller.js)' })
        }
      } else if (!isEntry) {
        violations.push({ file: f, pattern: 'DOM event binding outside controller (*.controller.js)' })
      }
