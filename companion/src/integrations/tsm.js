...
// Per-realm cache
const cache = new Map() // key: realmSlug, value: { ts, data }
const nowSec = () => Math.floor(Date.now() / 1000)

function normalizePriceMapTSM(payload) {
  if (!payload) {return {}}
...
export async function getTSMPriceMap(itemIds = [], realmSlugOverride) {
  const cfg = loadConfig()
  const apiKey = cfg.TSM_API_KEY
  const realm = (realmSlugOverride || cfg.REALM_SLUGS[0])
  const ttl = Math.max(0, Number(cfg.INTEGRATIONS_TTL || 900))
  const urlTpl = cfg.TSM_API_URL || ''
  ...
  try {
    ...
    const map = normalizePriceMapTSM(data)
    ...
    console.error('[EG] TSM fetch failed', err?.response?.data || err?.message)
    return entry?.data || {}
  }
}

export function getTSMStatus(realmSlugOverride) {
  const cfg = loadConfig()
  const realm = (realmSlugOverride || cfg.REALM_SLUGS[0])
  const entry = cache.get(realm)
  return {
    realm,
    configured: !!(cfg.TSM_API_URL && cfg.TSM_API_KEY),
