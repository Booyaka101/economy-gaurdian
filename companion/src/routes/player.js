...
export default function registerPlayerRoutes(app, _deps = {}) {
  ...
  let saveModels = function (m) {
    try {
      ensureDir(modelDir);
      fs.writeFileSync(modelPath, JSON.stringify(m, null, 2));
    } catch {}
  };

  // Allow dependency injection for tests
  if (_deps && typeof _deps === 'object') {
    if (typeof _deps.loadStore === 'function') {
      loadStore = _deps.loadStore;
    }
    if (typeof _deps.saveStore === 'function') {
      saveStore = _deps.saveStore;
    }
    if (typeof _deps.loadModels === 'function') {
      loadModels = _deps.loadModels;
    }
    if (typeof _deps.saveModels === 'function') {
      saveModels = _deps.saveModels;
    }
  }
    ...
    try {
      ...
      for (const [realm, chars] of Object.entries(body.realms || {})) {
        ...
        for (const [charName, buckets] of Object.entries(chars || {})) {
          const dst = (db.realms[realm][charName] = db.realms[realm][charName] || {
            postings: [],
            sales: [],
            payouts: [],
            buys: [],
          ...
          try {
            if (sqlite.isEnabled()) {
              sqlite.upsertBuckets(realm, charName, buckets);
            }
          } catch {}
        }
      }
  ...
  router.get('/player/payouts/unmatched', (req, res) => {
    try {
      ...
      const olderThanMin = Math.max(
        10,
        Math.min(7 * 24 * 60, Number(req.query.olderThanMin || 120)),
      );
      ...
      const paidKeys = new Set(
        payouts.map((p) => p.saleId || `${p.itemId}|${p.t}|${p.qty}|${p.unit}`),
      );
      const pending = [];
      ...
      try {
        console.error('[player] unmatched_failed', e);
      } catch {}
      return res.status(500).json({ error: 'unmatched_failed', message: e?.message || String(e) });
    }
  });
    ...
    try {
      ...
      if (type === 'all' || type === 'postings') {
        for (const p of scope.postings) {
          ...
          if (!Number.isFinite(t) || t * 1000 < sinceTs) {
            continue;
          }
          pushRow({
            type: 'posting',
      ...
      try {
        console.error('[player] ledger_failed', e);
      } catch {}
      return res.status(500).json({ error: 'ledger_failed', message: e?.message || String(e) });
    }
  });
  ...
  router.get('/player/summary', (req, res) => {
    try {
      ...
      for (const s of sales) {
        ...
        if (!Number.isFinite(ms) || ms < since) {
          continue;
        }
        const qty = Number(s.qty || 0);
        const unit = Number(s.unit || s.unitPrice || s.price || 0);
        ...
        const net = gross - cut;
        const k = dayKey(ms);
        addDay(k, 'gross', gross);
        addDay(k, 'ahCut', cut);
        addDay(k, 'netSales', net);
        incDay(k, 'salesCount');
      }
      for (const p of payouts) {
        ...
        if (Number.isFinite(net)) {
          const k = dayKey(ms);
          addDay(k, 'netPayouts', net);
          incDay(k, 'payoutsCount');
        }
      }
      const days = [...byDay.entries()].map(([day, v]) => ({
        ...
        ahCut: v.ahCut,
        netSales: v.netSales,
        netPayouts: v.netPayouts,
        salesCount: v.salesCount,
        payoutsCount: v.payoutsCount,
      }));
      ...
      try {
        console.error('[player] summary_failed', e);
      } catch {}
    ...
    try {
      ...
      for (const r of Object.keys(realms)) {
        ...
        for (const c of Object.keys(chars)) {
          ...
          if (last > best.lastTs) {
            best = { realm: r, character: c, lastTs: last };
          }
        }
      }
      return res.json({ current: best });
    } catch (e) {
      return res.status(500).json({ error: 'current_failed', message: e?.message || String(e) });
    }
  });
    ...
    try {
      ...
      if (sqlite.isEnabled()) {
        try {
          ...
          try {
            console.warn(
              '[player] sqlite queryStats failed, falling back to JSON:',
              e?.message || e,
            );
          } catch {}
    ...
    try {
      ...
      if (sqlite.isEnabled()) {
        try {
          ...
          try {
            console.warn(
              '[player] sqlite queryAwaiting failed, falling back to JSON:',
              e?.message || e,
            );
          } catch {}
          // fall through to JSON path below
      ...
      const lookbackMs = Math.max(
        60 * 60 * 1000,
        Math.min(7 * 24 * 60 * 60 * 1000, windowMin * 60 * 1000 * 6),
      );
      const paidKeys = new Set();
      for (const p of payouts) {
        const tsMs = Number(p.t || p.time || 0) * 1000;
        if (!Number.isFinite(tsMs) || tsMs < cutoff - lookbackMs) {
          continue;
        }
      ...
      return res.status(500).json({ error: 'awaiting_failed', message: e?.message || String(e) });
    }
  });
  ...
  function rebuildModels(db, sinceDays = 30) {
    ...
    for (const r of Object.keys(realms)) {
      for (const c of Object.keys(realms[r] || {})) {
        ...
        for (const s of sales) {
          ...
          if (!Number.isFinite(tMs) || tMs < sinceTs) {
            continue;
          }
          const itemId = Number(s.itemId);
          const qty = Number(s.qty || 0);
          const unit = Number(s.unit || s.unitPrice || s.price || 0);
          if (!Number.isFinite(itemId) || !Number.isFinite(qty) || !Number.isFinite(unit)) {
            continue;
          }
          const rec = (items[itemId] = items[itemId] || {
            units: [],
            qty: 0,
            gross: 0,
            count: 0,
    ...
    for (const [itemId, rec] of Object.entries(items)) {
      ...
      items[itemId] = {
        count: rec.count,
        qty: rec.qty,
        meanUnit: Math.round(mean),
        p25,
        p50,
        p75,
        bestHours,
        hourHist: rec.hourHist,
      };
    }
  ...
  router.get('/player/recommend/price', (req, res) => {
    try {
      ...
      if (!models.updatedAt) {
        models = rebuildModels(loadStore(), 30);
      }
      ...
      const stack = Math.max(
        1,
        Math.min(maxStack, rec?.qty && rec?.count ? Math.round(rec.qty / rec.count) : maxStack),
      );
      ...
      return res.json({
        itemId,
        targetHours,
        recommended: { unit, stack, gross, ahCut, net, expectedETA },
        basis: rec || null,
      });
    } catch (e) {
      return res.status(500).json({ error: 'recommend_failed', message: e?.message || String(e) });
    }
  });
  ...
  router.get('/player/insights', (req, res) => {
    try {
      ...
      const realms = db.realms || {};
      for (const r of Object.keys(realms)) {
        if (realmFilter && r !== realmFilter) {
          continue;
        }
        for (const c of Object.keys(realms[r] || {})) {
          if (charFilter && c !== charFilter) {
            continue;
          }
          const sales = realms[r][c]?.sales || [];
          for (const s of sales) {
            const tMs = Number(s.t || s.time || 0) * 1000;
            if (!Number.isFinite(tMs) || tMs < since) {
              continue;
            }
            const itemId = Number(s.itemId);
            const qty = Number(s.qty || 0);
            ...
            const net = gross - cut;
            if (!byItem[itemId]) {
              byItem[itemId] = { itemId, salesCount: 0, qty: 0, gross: 0, net: 0 };
            }
      ...
      for (const it of Object.values(models.items || {})) {
        ...
        for (let h = 0; h < 24; h++) {
          hourScore[h] += Number(hist[h] || 0);
        }
      }
      const bestHours = hourScore
        .map((v, i) => ({ i, v }))
        .sort((a, b) => b.v - a.v)
        .slice(0, 5)
        .map((x) => x.i);
      ...
      return res.status(500).json({ error: 'insights_failed', message: e?.message || String(e) });
    }
  });
  ...
  router.get('/player/characters', (req, res) => {
    try {
      ...
      return res.status(500).json({ error: 'characters_failed', message: e?.message || String(e) });
    }
  });
  ...
  router.get('/player/top-items', (req, res) => {
    try {
      ...
      return res.status(500).json({ error: 'top_items_failed', message: e?.message || String(e) });
    }
  });
  ...
  router.get('/player/recommend/window', (req, res) => {
    try {
      ...
      const ranked = hourScore
        .map((v, i) => ({ hour: i, score: v }))
      ...
      return res.status(500).json({ error: 'window_failed', message: e?.message || String(e) });
    }
  });
  ...
  router.get('/player/accounting/status', (req, res) => {
    try {
      ...
      return res.status(500).json({ error: 'status_failed', message: e?.message || String(e) });
    }
  });
