...
export default function registerCatalogRoutes(app, deps) {
  ...
  function checkRate(req, res) {
    ...
    let st = rl.get(key)
    if (!st || now >= st.resetAt) { st = { count: 0, resetAt: now + RL_WINDOW_MS }; rl.set(key, st) }
    st.count += 1
    if (st.count > RL_MAX) {
      setCORS(res)
  ...
  app.options('/catalog/*', (req, res) => { setCORS(res); res.sendStatus(204) })
  ...
  app.get('/catalog/search', (req, res) => {
    try {
      ...
      if (!rawQ) { res.status(400).json({ error: 'missing_query' }); return }
      ...
      for (const it of items) {
        ...
        let score = null
        if (isNum && id === qId) {score = 0}
        else if (toks.every(t => lo.startsWith(t))) {score = 1}
        else if (toks.every(t => lo.includes(t))) {score = 2}
        if (score != null) {results.push({ id, name, score })}
      }
      ...
      res.json({ count: out.length, items: out }); return
    } catch (e) {
      setCORS(res)
      res.status(500).json({ error: 'catalog_search_failed', message: e?.message || String(e) }); return
    }
  })
  ...
  function pickName(it) {
    ...
    if (!nm) {return ''}
    return (
      nm.en_GB || nm.en_US || nm['en-US'] || nm.en || nm.default || nm.name || nm.label || nm.display || ''
    )
  }
  ...
  app.get('/catalog/item/:id', (req, res) => {
    try {
      ...
      if (!it) { res.status(404).json({ error: 'not_found', id }); return }
      res.json({ id, name: pickName(it), item: it }); return
    } catch (e) {
      setCORS(res)
      res.status(500).json({ error: 'catalog_item_failed', message: e?.message || String(e) }); return
    }
  })
  ...
  app.post('/catalog/bulk', (req, res) => {
    try {
      ...
      if (!ids.length) { res.status(400).json({ error: 'missing_ids' }); return }
      ...
      for (const id of want) {
        const it = map.get(id)
        if (it) {out.push({ id, name: pickName(it), item: it })}
      }
      res.json({ count: out.length, items: out, truncated: ids.length > MAX }); return
    } catch (e) {
      setCORS(res)
      res.status(500).json({ error: 'catalog_bulk_failed', message: e?.message || String(e) }); return
    }
  })
  ...
  app.get('/catalog/autocomplete', (req, res) => {
    try {
      ...
      for (const it of items) {
        ...
        const lo = String(name || '').toLowerCase()
        let score = null
        if (isNum && id === qId) {score = 0} // highest priority
        else if (lo.startsWith(q)) {score = 1}
        else if (lo.includes(q)) {score = 2}
        if (score != null) {results.push({ id, name, score })}
      }
      results.sort((a,b) => a.score - b.score || String(a.name).localeCompare(String(b.name)))
      const out = results.slice(0, limit).map(r => ({ id: r.id, name: r.name }))
      ...
      setCORS(res)
      res.status(500).json({ error: 'catalog_autocomplete_failed', message: e?.message || String(e) }); return
    }
  })
}
